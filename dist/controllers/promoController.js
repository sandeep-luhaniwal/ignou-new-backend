...
exports.deletePromoCodeAdmin = exports.togglePromoCodeStatusAdmin = exports.updatePromoCodeAdmin = exports.createPromoCodeAdmin = exports.getAllPromoCodesAdmin = exports.validatePromoCode = void 0;
...
const ensureDefaultPromos = async () => {
    try {
        const count = await PromoCode_1.default.countDocuments();
        if (count === 0) {
            await PromoCode_1.default.create([
                {
                    code: "IGNOU10",
...
const getAllPromoCodesAdmin = async (req, res) => {
    try {
        await ensureDefaultPromos();
        const promoCodes = await PromoCode_1.default.find().sort({ createdAt: -1 });
        res.json({
            success: true,
            data: promoCodes
        });
...
exports.getAllPromoCodesAdmin = getAllPromoCodesAdmin;
// 3. Create promo code (Admin)
const createPromoCodeAdmin = async (req, res) => {
    try {
        const { code, description, discountType, discountValue, minOrderAmount, maxDiscount, isActive, usageLimit, expiresAt } = req.body;
        if (!code || !discountValue) {
            return res.status(400).json({ message: "Code and discount value are required" });
        }
        const cleanCode = code.trim().toUpperCase();
        const existing = await PromoCode_1.default.findOne({ code: cleanCode });
        if (existing) {
            return res.status(400).json({ message: `Promo code "${cleanCode}" already exists` });
        }
        const newPromo = await PromoCode_1.default.create({
            code: cleanCode,
...
exports.createPromoCodeAdmin = createPromoCodeAdmin;
// 4. Update promo code (Admin)
const updatePromoCodeAdmin = async (req, res) => {
    try {
        const { id } = req.params;
        const promo = await PromoCode_1.default.findById(id);
        if (!promo) {
        ...
        if (discountType !== undefined)
            promo.discountType = discountType === "flat" ? "flat" : "percentage";
        if (discountValue !== undefined)
            promo.discountValue = Number(discountValue);
        if (minOrderAmount !== undefined)
            promo.minOrderAmount = Number(minOrderAmount);
        if (maxDiscount !== undefined)
            promo.maxDiscount = Number(maxDiscount);
        if (isActive !== undefined)
            promo.isActive = Boolean(isActive);
        if (usageLimit !== undefined)
            promo.usageLimit = Number(usageLimit);
        if (expiresAt !== undefined)
            promo.expiresAt = expiresAt ? new Date(expiresAt) : undefined;
        await promo.save();
        res.json({
            success: true,
...
exports.updatePromoCodeAdmin = updatePromoCodeAdmin;
// 5. Toggle Active/Block status of promo code (Admin)
const togglePromoCodeStatusAdmin = async (req, res) => {
    try {
        const { id } = req.params;
        const promo = await PromoCode_1.default.findById(id);
        if (!promo) {
...
exports.togglePromoCodeStatusAdmin = togglePromoCodeStatusAdmin;
// 6. Delete promo code (Admin)
const deletePromoCodeAdmin = async (req, res) => {
    try {
        const { id } = req.params;
        const promo = await PromoCode_1.default.findByIdAndDelete(id);
        if (!promo) {
            return res.status(404).json({ message: "Promo code not found" });
        }
        res.json({
...
exports.deletePromoCodeAdmin = deletePromoCodeAdmin;
