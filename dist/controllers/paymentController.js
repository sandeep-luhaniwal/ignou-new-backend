...
exports.getPaymentGraphAnalytics = exports.getPaymentHistory = void 0;
const Order_1 = __importDefault(require("../models/Order"));
const User_1 = __importDefault(require("../models/User"));
// Helper function to build 14-day and 30-day filled time series
const buildFilledTimeSeries = (dailyAgg, daysCount = 14) => {
    const map = {};
...
const getPaymentHistory = async (req, res) => {
    try {
        ...
        const status = (req.query.status || req.query.paymentStatus || "All").trim();
        const deliveryType = (req.query.deliveryType || "All").trim();
        const query = {};
        // Status filter
        ...
        const [total, payments, totalAll, paidAll, pendingAll, failedAll, refundedAll, revenueAgg, refundAgg, todayRevenueAgg, monthRevenueAgg, dailyChartAgg, deliveryTypeAgg] = await Promise.all([
            ...
            Order_1.default.countDocuments({ paymentStatus: { $regex: /^paid$/i } }),
            Order_1.default.countDocuments({ paymentStatus: { $regex: /^pending$/i } }),
            Order_1.default.countDocuments({ paymentStatus: { $regex: /^failed$/i } }),
            Order_1.default.countDocuments({
                $or: [
        ...
        const formattedPayments = payments.map((order) => ({
            ...
            itemCount: (order.items || []).reduce((acc, curr) => acc + (curr?.quantity || 1), 0),
...
const getPaymentGraphAnalytics = async (req, res) => {
    try {
        ...
        const [dailyAgg, monthlyAgg, statusAgg, deliveryAgg, kpis] = await Promise.all([
...
exports.getPaymentGraphAnalytics = getPaymentGraphAnalytics;
